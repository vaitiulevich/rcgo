from categories.models import SubCategory, TemporarilyLinksOptax
...
def get_parse_links():
    ...
    for subcategory in subcategories:
        if subcategory.optax_link is not None:
            for link in subcategory.optax_link.split('\n'):
                if link != '':
...
def save_temporarily_links(links, subcategory):
    TemporarilyLinksOptax.objects.create(links=links, subcategory=subcategory)
...
def get_all_pages(link, pages_num, subcategory):
    ...
    for i in range(1, pages_num + 1):
        ...
        links = soup.find_all('div', class_='item product sku')
        for product_link in links:
            link_result = f"https://optax.ru{product_link.find('div', class_='productColText').a['href']}"
